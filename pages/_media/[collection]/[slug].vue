	...
	return {
		posts: {
			...
			propMap: (data) => ({
				...
				authorName: data.author ? `${data.author.first_name} ${data.author.last_name}` : '',
...
const {
	...
	path,
	() => {
		return $directus.request(params.collection), mapping(params.slug)[params.collection].query;
	},
	{
...
<template>
	<component :is="components[type]" v-bind="mapping(params.slug)[params.collection].propMap(page)" />
</template>
