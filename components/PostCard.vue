...
<template>
	<figure class="flex flex-col group">
		...
		<NuxtLink class="h-full" :href="`/posts/${post.slug}`">
			<!-- Icon -->
			<p class="mt-5 font-serif text-xl font-semibold text-gray-900 dark:text-white group-hover:text-accent">
				{{ post.title }}
			</p>
		...
		<VAvatar v-if="isObject(post.author)" class="mt-4" size="sm" :author="post.author" />
	</figure>
</template>
