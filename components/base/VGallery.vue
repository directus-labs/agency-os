...
<template>
	...
	<div class="gap-4 mt-4 md:columns-3">
		...
		>
			...
			>
				<img :src="fileUrl(item.id)" class="object-cover w-full transition duration-300 group-hover:scale-110" />
				<div
				...
				>
					<Icon name="heroicons:magnifying-glass-plus" class="w-12 h-12 text-gray-500 dark:text-white" />
				</div>
			</div>
		</button>
	</div>
	...
	>
		...
		<div class="absolute z-50 hidden font-mono text-white md:block top-4 left-4">
		...
		<div class="absolute z-50 font-mono text-white bottom-4 right-4">
			<VBadge v-for="(item, itemIdx) in currentItem.tags" :key="itemIdx" size="lg" class="rounded-br-xl">
				{{ item }}
			</VBadge>
		</div>
			...
			<div class="flex items-center justify-center w-full h-full">
				...
				<div class="relative flex items-center justify-center">
					<div class="relative w-full h-full p-20">
						<!-- Metadata -->
						<div class="flex">
							<p class="inline-block px-6 py-2 font-serif font-bold text-white bg-gray-900 track rounded-tl-3xl">
								{{ currentItem.title }}
							</p>
							<p
