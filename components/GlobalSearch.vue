	...
	<Combobox as="div" class="" v-model="selected">
		<div class="relative w-full mt-2">
			...
			<button
				...
				@click="query = ''"
			>
				<Icon name="heroicons:x-mark" class="w-5 h-5 text-gray-400" aria-hidden="true" />
			</button>
			...
			>
				<div class="relative px-2 space-y-2">
					<div v-if="loading" class="flex items-center justify-center w-full h-12">
						<VLoading name="heroicons:refresh" class="w-8 h-8 text-gray-700 dark:text-white" />
					</div>
					<ComboboxOption v-for="hit in results" :key="hit.id" :value="hit" as="template" v-slot="{ active, selected }">
						<li
							:class="[
						...
						>
							<img
								...
								alt=""
							/>
							<div v-else class="w-10 h-10 bg-gray-200 rounded-tr rounded-bl dark:bg-gray-900"></div>
							<p class="font-mono text-sm font-semibold text-gray-900 dark:text-white group-hover:text-accent">
								{{ truncateString(hit.title, 50) }}
							</p>
						</li>
					</ComboboxOption>
					<div class="w-full py-2 font-mono text-center text-gray-500 border-t dark:border-t-gray-700">
