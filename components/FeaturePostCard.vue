...
<template>
	<figure class="relative p-2 space-x-2 border-2 md:flex border-accent rounded-bl-3xl rounded-tr-3xl]">
		<NuxtLink class="group" :href="`/posts/${post.slug}`">
			<div :class="['relative overflow-hidden h-[300px] w-[300px] duration-300 rounded-bl-2xl ']">
				<img
		...
		<div class="relative px-8 overflow-hidden transition duration-300 rounded-tl-3xl rounded-tr-3xl group">
			...
			<VAvatar v-if="post.author" class="relative mt-4" size="sm" :author="post.author" />
		</div>
	</figure>
</template>
