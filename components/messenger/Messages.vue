...
const { $directus, $readItems, $createItem, $directusWs } = useNuxtApp();
const { fileUrl } = useFiles();
const session = useCookie('session');
const { selectedConversation, connect, isCurrentUser, subscribeToMessages } = useChat();
...
<template>
	<div class="relative flex flex-col h-full overflow-hidden">
		<template v-if="!selectedConversation">
			...
			>
				<VButton class="" @click="createNewConversation()" variant="primary">Send Us A Message</VButton>
			</div>
		</template>
		<MessengerConversation v-if="selectedConversation" :id="selectedConversation" />
	</div>
</template>
