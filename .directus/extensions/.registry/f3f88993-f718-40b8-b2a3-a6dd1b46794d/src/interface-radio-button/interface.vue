<template>
	...
	>
		<v-radio
			...
			:label="item.text"
			:disabled="disabled"
			:icon-on="getIcon(item.value,item.iconOn)"
			:icon-off="getIcon(item.value, item.iconOff)"
			:model-value="value"
...
const getIcon = (value: string, icon: string) => value?.match(/facebook|twitter|google|pinterest|youtube|yandex/) ? value : (icon || 'no')
</script>
